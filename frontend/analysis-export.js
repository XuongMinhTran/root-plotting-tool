...
function report(doc,ids,options={}){
 ...
 for(const [k,g] of (doc.inputs.simultaneous_fits||[]).entries()){
  const r=g.result?.response;if(!r?.params?.length||!(opts.fit||opts.graphs)||!g.members.some(m=>ids.includes(m.datasetId)))continue;
  const stem='simultaneous-'+(k+1),stale=g.result.sourceSignature&&g.result.sourceSignature!==scope.WorkspaceStore.groupSignature(g,doc.inputs.datasets);
  let text='\\section{Simultaneous fit: '+tex(g.name)+'}\n'+(stale?'\\textbf{The inputs have changed since this fit. This result is a saved snapshot.}\\par\n':'');
  if(opts.fit){
   text+=tex(r.status_message)+'\\par\nDatasets fitted together with shared parameters: '+tex(r.datasets.map(d=>d.name).join(', '))+'. Total chi-square: '+tex(r.chi2)+'; NDF: '+tex(r.ndf)+'; probability: '+tex(r.prob)+'; free parameters: '+tex(r.n_free)+'.\\par\n'+(r.x_error_note?tex(r.x_error_note)+'\\par\n':'');
   const sets=r.datasets.map(d=>[d.name,d.formula,d.range[0]+' to '+d.range[1],d.n_points,d.chi2]);files[stem+'-datasets.csv']=csv([['Dataset','Function','Fit range','Points','Chi-square'],...sets]);text+=table(['Dataset','Function','Fit range','Points','Chi-square'],sets);
   const rows=r.params.map(p=>[p.name,p.kind==='shared'?'shared':p.kind==='fixed'?'fixed ('+p.dataset_name+')':p.dataset_name,p.value,p.fixed?'':p.error]);files[stem+'-parameters.csv']=csv([['Parameter','Belongs to','Value','Standard uncertainty'],...rows]);text+=table(['Parameter','Belongs to','Value','Standard uncertainty'],rows);
   if(r.covariance?.length){const names=r.params.map(p=>p.label||p.name),matrix=r.covariance.map((row,i)=>row.map((v,j)=>{const den=Math.sqrt(r.covariance[i][i]*r.covariance[j][j]);return den?v/den:'';}));files[stem+'-correlation.csv']=csv([['Parameter',...names],...matrix.map((row,i)=>[names[i],...row])]);text+='Correlation matrix: '+tex(stem+'-correlation.csv')+'.\\par\n';if(names.length<=6)text+=table(['Parameter',...names],matrix.map((row,i)=>[names[i],...row.map(v=>v===''?'—':Number(v).toPrecision(4))]));}
  }
  if(opts.graphs&&r.canvas_json)text+='\\begin{figure}[ht]\\centering\\includegraphics[width=\\linewidth]{figures/'+stem+'.png}\\caption{'+tex(g.name)+(stale?' (saved fit; inputs subsequently changed)':'')+'}\\end{figure}\n';
  sections.push(text);
 }
...
async function images(doc,ids,files,format='both'){
 const datasets=doc.inputs.datasets.filter(d=>ids.includes(d.id)&&d.result?.response?.canvas_json).map(d=>({name:d.name,stem:'figures/dataset-'+(doc.inputs.datasets.indexOf(d)+1),result:d.result}));
 for(const [k,g] of (doc.inputs.simultaneous_fits||[]).entries())if(g.result?.response?.canvas_json&&g.members.some(m=>ids.includes(m.datasetId)))datasets.push({name:g.name,stem:'figures/simultaneous-'+(k+1),result:g.result});
 ...
 for(const d of datasets){const host=document.createElement('div');host.className='export-render-host';document.body.append(host);let painter;
  try{painter=await scope.JSROOT.draw(host,scope.JSROOT.parse(JSON.stringify(d.result.response.canvas_json)),'');const canvas=painter.getCanvPainter?.()||painter;const svg=await canvas.produceImage(true,'svg');if(!svg)throw Error('Could not export the plot for '+d.name);const stem=d.stem;if(format!=='png')files[stem+'.svg']=svg;
...
function open(doc){
 ...
 const calculations=(doc.objects||[]).filter(o=>o.kind==='calculation'),hasGraphs=doc.inputs.datasets.some(d=>d.result?.response?.canvas_json)||(doc.inputs.simultaneous_fits||[]).some(g=>g.result?.response?.canvas_json);
 ...
 dlg.querySelector('form').onsubmit=async e=>{
  ...
  try{
   ...
   if(options.graphs&&!doc.inputs.datasets.some(d=>ids.includes(d.id)&&d.result?.response?.canvas_json)&&!(doc.inputs.simultaneous_fits||[]).some(g=>g.result?.response?.canvas_json&&g.members.some(m=>ids.includes(m.datasetId))))throw Error('The selected datasets have no saved graphs. Run a fit or turn off Graphs.');
