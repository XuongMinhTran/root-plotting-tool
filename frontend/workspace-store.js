...
function signature(d){const n=list(d.x).length,errors=s=>{const a=list(s);return a.length===1?Array(n).fill(a[0]):a;};return JSON.stringify({type:d.analysis_type||'xy',x:list(d.x),y:list(d.y),ex:errors(d.ex),ey:errors(d.ey),histogram:d.histogram||null,formula:d.fit?.formula||'',...(d.exclusions?.length?{exclusions:d.exclusions}:{}),...(d.mv?{mv:d.mv}:{})});}
...
function normalize(input){
 ...
 for(const d of doc.inputs.datasets){
  ...
  d.name=String(d.name||'Dataset');d.analysis_type=d.analysis_type===''?'':['histogram','multivariate'].includes(d.analysis_type)?d.analysis_type:'xy';d.fit=d.fit||{...fitDefaults(),formula:doc.inputs.formula||'[0]*x+[1]',param_names:doc.inputs.param_names||'',initial_guesses:doc.inputs.initial_guesses||''};
 }
